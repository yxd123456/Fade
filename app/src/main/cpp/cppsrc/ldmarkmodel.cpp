...
cv::Mat ldmarkmodel::predict(cv::Mat& src){
    cv::Mat grayImage;
    if(src.channels() == 1){
        grayImage = src;
...
int ldmarkmodel::track(cv::Mat& src, cv::Mat& current_shape, bool isDetFace, cv::Size minSz){
	//imshow("SRC", src);
    cv::Mat grayImage;
    if(src.channels() == 1){
        grayImage = src;
...
//    }
    ...
    if(isDetFace || faceBox.area()<100){
        std::vector<cv::Rect> mFaceRects;
        face_cascade.detectMultiScale(grayImage, mFaceRects, 1.3, 3, 0, minSz);
        if(mFaceRects.size() <=0){
            current_shape = cv::Mat();
            return SDM_ERROR_FACENO;
        }
...
void ldmarkmodel::drawPose(cv::Mat& img, cv::Mat& current_shape, float lineL)
{
    if(current_shape.empty())
        return;
    static const int samplePdim = 7;
