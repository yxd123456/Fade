...
public:
	...
	void loadFaceDetModelFile(std::string filePath = "haar_roboman_ff_alt2.xml"); //"haarcascade_frontalface_alt_tree.xml");

    void train(std::vector<ImageLabel> &mImageLabels);

    cv::Mat predict(cv::Mat& src);

	int  track(cv::Mat& src, cv::Mat& current_shape, bool isDetFace = false, cv::Size minSz = cv::Size(100, 100));
    ...
    void drawPose(cv::Mat& img, cv::Mat& current_shape, float lineL=50);
